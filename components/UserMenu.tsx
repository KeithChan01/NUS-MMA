  ...
  return (
    <>
      <div className="relative">
        ...
        >
          {avatar ? (
            <img src={avatar} alt={name} className="w-8 h-8 rounded-full border border-gray-600" />
          ) : (
          ...
          <>
            ...
            <div className="absolute right-0 mt-2 w-48 bg-gray-800 border border-gray-700 rounded-xl shadow-xl z-20 overflow-hidden">
              ...
              <button
                ...
                className="w-full text-left px-4 py-3 text-sm text-gray-300 hover:bg-gray-700 hover:text-white transition-colors border-t border-gray-700"
              >
                Sign out
              </button>
            </div>
      ...
      {editingProfile && (
        <ProfileSetup existing={profile} onClose={() => setEditingProfile(false)} />
      )}
    </>
  );
}
